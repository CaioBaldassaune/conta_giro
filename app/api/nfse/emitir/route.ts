...
// Emite a NFS-e de um rascunho (Sefin Nacional ou WebISS, conforme a configuração da empresa).
export async function POST(request: Request) {
  try {
    ...
    const webiss = "provedor" in r && r.provedor === "webiss";
    const ambiente = r.ambiente === "producao" ? "produção" : webiss ? "homologação do WebISS (sem valor fiscal)" : "produção restrita (sem valor fiscal)";
    ...
    return Response.json({
      ...
      message: `NFS-e ${r.numeroNfse} autorizada em ${ambiente}.${r.danfse ? " XML e DANFSe" : " XML"} arquivados em Documentos.${webiss && "link" in r ? ` Visualizar: ${r.link}` : ""}${alertas}`,
