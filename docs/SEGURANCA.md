...
| Portas dos portais | Sem login → tela de entrada; cliente logado que abre `/contador` vai para `/cliente` | `proxy.ts` |
...
| Auditoria | Cada gravação registra quem, quando e o quê em `registros_auditoria` | `aplicar_alteracoes` e rotas |
...
| `api/onboarding` | criar a empresa do autocadastro (o cliente ainda não tem vínculo) | usuário logado, não é da equipe, CNPJ válido e ainda não na carteira; uma entrada por usuário |
